...
Output: portfolio/Voxa+-case-study.pdf
...
OUT_NAME = "Voxa+-case-study.pdf"

DEEP = (36, 19, 72)          # Voxa+ VIOLET_DEEP #241348
ACCENT = (230, 162, 60)      # Voxa+ AMBER #E6A23C

COVER = dict(
    headline=["Churn nearly doubled.", "It wasn't one cause —", "it was three, stacked."],
    sub="How I separated the acute trigger from the amplifier from the pre-existing "
        "drain — and ruled out the obvious suspect.",
    footer="Fictional SVOD streaming service · synthetic data · real methodology",
)

PAGES = [
    dict(section="01 — THE PROBLEM", kind="stat",
         stat="2×", bold="gross monthly churn, starting month 34 of 36 — and still elevated.",
         body="Leadership needed a root cause before the next board meeting: was it "
              "content, price, acquisition quality, billing, the app, or the market?"),
    dict(section="02 — THE APPROACH", kind="metrics",
         bold="Test every plausible cause — market, price, app, acquisition, payments — "
              "and rule out what isn't it.",
         metrics=[
             ("PRIMARY METRIC", "Gross monthly churn %, decomposed by market / cohort / "
                                 "device / tier"),
             ("METHOD", "Every result computed twice — SQL and pandas — and asserted equal"),
         ]),
    dict(section="03 — THE SETUP", kind="cards",
         bold="Ten questions, dual-track, parity-checked.",
         cards=[("36", "MONTHS"), ("3", "MARKETS"), ("~70K", "ACTIVE SUBSCRIBERS")],
         body="DuckDB SQL and pandas, independently, from market decomposition to an "
              "explicit \u201cpayments ruled out\u201d finding."),
    dict(section="04 — WHAT WE FOUND", kind="stat_callout",
         stat="3", stat_sub="overlapping causes, none sufficient alone",
         callout="A connected-TV app regression (the acute trigger), a Brazil price hike "
                 "(the amplifier — engaged subscribers untouched), and a Mexico "
                 "cohort-quality drag (the pre-existing drain)."),
    dict(section="05 — THE CATCH", kind="bold_callout",
         bold="Payments were the obvious suspect. They were tested first — and ruled out.",
         callout="Failure rate, dunning recovery, and involuntary churn are all flat. "
                 "Whatever changed, it wasn't billing."),
]

CALL = dict(
    section="06 — THE CALL",
    headline=["Fix the app regression first —", "it's the acute trigger.",
              "The other two need slower fixes."],
