"""
_drive_pbir.py — build the Voxa+ report from powerbi/_report_layout.json using pbir.

Called by build_report.sh (which does the env / abort-if-Desktop-running checks).
Idempotent: wipes and recreates powerbi/VoxaChurnDiagnosis.Report each run.
"""
...
def scaffold():
    ...
    # apply powerbi/theme/theme.json  (serialize to a .Theme folder, then build into the report)
    tdir = PBI / "_voxa.Theme"
    if tdir.exists():
        shutil.rmtree(tdir, ignore_errors=True)
    rc, _ = run("theme", "serialize", str(THEME), "-o", str(tdir), check=False)
    rc2, out = run("theme", "build", str(tdir), "-o", RPT, "-f", "--clean", check=False)
    shutil.rmtree(tdir, ignore_errors=True)
    print(f"   theme: serialize rc={rc} · build rc={rc2}"
          + ("" if rc2 == 0 else "  (apply powerbi/theme/theme.json manually in Desktop)"))
...
def page_bundle(pg) -> list[dict]:
    ...
    for it in pg["kpis"]:
        b.append({"visual_type": "card", "name": _slug(pg['key'], it['title'], 'v'),
                  "x": it["x"], "y": it["y"], "width": it["w"], "height": it.get("val_h", 70),
...
def post_pass():
    for pg in LAYOUT["pages"]:
        for v in pg["visuals"]:
            ...
            # trim the market-launch ramp from every month-on-x trend visual: BR/MX/US
            # all reach a stable base by window-month 18 (= 2025-03), so the
            # tiny-denominator churn spikes of the launch months stop dominating the
            # y-axis and hiding the real step. Filter on year_month (string) not
            # window_month_idx: pbir serialises Advanced-filter literals as strings, so
            # a numeric column gets a broken `int >= '18'` comparison that Power BI
            # rejects ("Algo está errado com um ou mais filtros").
            if (v.get("roles") or {}).get("Category") == "dim_date.year_month":
                run("add", "filter", "dim_date", "year_month",
                    "-v", vpath(pg["name"], v["name"]),
                    "--type", "Advanced", "--operator", "GreaterThanOrEqual",
                    "--values", "2025-03", "--no-validate", check=False)
        for it in pg["kpis"]:
            if not it.get("delta"):
                continue
    ...
    for pg in LAYOUT["pages"]:
        ...
        # .color -- the old path silently failed every run() (check=False),
        # leaving the frame with no explicit fill and defaulting to the
        # theme's first data color (#4B2E83, a violet) instead of white.
        run("set", f"{fr}.fill.fillColor", "--value", "#FFFFFF", check=False)
        # roundEdge lives under the shape-specific "shape" object, not at
        # the visual root -- same silent-failure bug as fillColor above.
        run("set", f"{fr}.shape.roundEdge", "--value", "18", check=False)
        run("set", f"{fr}.dropShadow.show", "--value", "false", check=False)
    # narrative cards -> wrap the text
    for nm in ("exec_insight", "insight_7"):
        for pg in LAYOUT["pages"]:
            run("set", f"{vpath(pg['name'], nm)}.wordWrap.show", "--value", "true", check=False)
    # report-level in-window filter
    run("add", "filter", "dim_date", "in_analysis_window", "-r", RPT, check=False)
