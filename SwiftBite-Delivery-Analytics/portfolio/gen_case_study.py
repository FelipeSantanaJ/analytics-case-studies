...
Output: portfolio/SwiftBite-Delivery-case-study.pdf
...
# ============================================================== CONFIG (per project) ===
PROJECT = "SWIFTBITE DELIVERY"
AUTHOR = "FELIPE SANTANA · SENIOR DATA & PRODUCT ANALYST"
OUT_NAME = "SwiftBite-Delivery-case-study.pdf"

DEEP = (17, 58, 44)          # SwiftBite GREEN_DEEP #113A2C
ACCENT = (232, 130, 60)      # SwiftBite TANGERINE #E8823C

COVER = dict(
    headline=["The test won on liquidity.", "I still said don't ship it."],
    sub="How a zone-hour courier bonus that worked got killed by its own price tag.",
    footer="Fictional food-delivery marketplace · synthetic data · real methodology",
)

PAGES = [
    dict(section="01 — THE PROBLEM", kind="stat",
         stat="6 of 12", bold="zones are structurally supply-short.",
         body="Demand outruns couriers at peak while other zones sit idle. Customers "
              "wait, then bail — ETA climbs and no-courier cancellations rise, at the "
              "same time couriers idle in the wrong zones."),
    dict(section="02 — THE TEST", kind="metrics",
         bold="Pay a per-delivery bonus in one named zone during the Friday/Saturday "
              "peak. See if liquidity improves — and what it costs.",
         metrics=[
             ("PRIMARY METRIC", "Fulfillment rate — share of demand delivered, by zone-day"),
             ("GUARDRAILS", "Incentive cost per incremental order · ETA p50/p90 · "
                             "no-courier cancel rate · courier earnings/hr · "
                             "neighbour-zone spillover"),
         ]),
    dict(section="03 — THE SETUP", kind="cards",
         bold="Randomized by zone-day, not by customer.",
         cards=[("12", "ZONES"), ("10", "WEEKS"), ("840", "ZONE-DAYS, 420/ARM")],
         body="The intervention is supply-side, so inference runs three ways — "
              "cluster-robust, randomization inference, wild-cluster bootstrap — to "
              "stay honest about only 12 clusters."),
    dict(section="04 — WHAT HAPPENED", kind="stat_callout",
         stat="+1.87pp", stat_sub="fulfillment-rate lift · cluster CI [+1.27, +2.46] · "
                                   "p = 3×10⁻⁴",
         callout="ETA p90 −6 minutes, no-courier cancels −0.36pp — concentrated in the "
                 "supply-short and balanced zones (interaction p = 0.004)."),
    dict(section="05 — THE CATCH", kind="bold_callout",
         bold="The lever works. Its price doesn't.",
         callout="R$281 per incremental delivered order against a R$10 contribution "
                 "margin — fails the guardrail by about 28×. A flat bonus pays every "
                 "delivery; only ~2% are incremental."),
]

CALL = dict(
    section="06 — THE CALL",
    headline=["Don't ship the flat bonus.", "Restructure it to reward",
              "incremental supply."],
