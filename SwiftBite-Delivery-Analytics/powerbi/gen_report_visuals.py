...
def kpis(page_key, items):
    ...
    y=200 clears the header wallpaper's divider (y=144, see assets/gen_logo.py
    _page_bg) and the slicer row below it (y=158..192) — cards used to start
    at y=132, on top of both.
    """
    out = []
    for i, (title, value, good) in enumerate(items):
        out.append({"title": title, "value": f"_Measures.{value}", "delta": None,
                    "good": good, "x": KPI_X[i], "y": 200, "w": KPI_W,
                    "delta_y": 276, "delta_h": 22})
    return out
...
LAYOUT = {
    ...
    # y=150/h=130 -> bottom=280, clearing the header wallpaper's divider (y=144)
    # with a 6px margin; slicers (y=158) then sit above cards (y=200, see kpis())
    # instead of overlapping them.
    "kpi_frame": {"x": 16, "y": 150, "w": 1248, "h": 130, "radius": 18},
    "slicers": [
        {"name": "sl-period", "field": "dim_date.date", "x": 24, "y": 158, "w": 320, "h": 34,
         "title": "Analysis period"},
        {"name": "sl-zone", "field": "dim_zone.zone_name", "x": 856, "y": 158, "w": 210, "h": 34,
         "title": "Zone"},
        {"name": "sl-tier", "field": "dim_zone.baseline_supply_stress_tier", "x": 1078, "y": 158,
    ...
    "pages": [
        {
            ...
            "visuals": [
                V("fulfillment_trend", "lineChart", 24, 280, 730, 250,
                  "Fulfillment rate over the window",
                  {"Category": "dim_date.date", "Y": m("Fulfillment Rate %")}),
                V("exec_insight", "card", 766, 280, 490, 250, "Read this",
                  {"Values": m("Exec Insight")}),
                V("lift_by_tier_exec", "clusteredBarChart", 24, 544, 490, 232,
                ...
                V("novelty_exec", "lineChart", 530, 544, 726, 232,
        ...
        },
        {
            ...
            "visuals": [
                V("lift_by_tier", "clusteredBarChart", 24, 280, 610, 232,
                  "Fulfillment lift by tier (pp) — heterogeneity",
                  {"Category": "dim_zone.baseline_supply_stress_tier", "Y": m("Fulfillment Lift (pp)")},
                  {"by": "dim_zone.baseline_supply_stress_tier", "dir": "asc"}),
                V("fulfillment_by_arm", "clusteredColumnChart", 646, 280, 610, 232,
                  "Fulfillment by arm (zone-day mean)",
                  {"Category": "fact_experiment_zone_day.arm", "Y": m("Zone-Day Fulfillment %")}),
                V("novelty_curve", "lineChart", 24, 526, 726, 224,
                  "Weekly fulfillment, treated vs control — launch bump vs settled",
                  {"Category": "fact_experiment_zone_block_week.experiment_week",
                   "Y": m("Weekly Fulfillment (Treated) %", "Weekly Fulfillment (Control) %")},
                  {"by": "fact_experiment_zone_block_week.experiment_week", "dir": "asc"}),
                V("guardrails", "tableEx", 762, 526, 494, 224, "Guardrails by arm",
        ...
        },
        {
            ...
            "visuals": [
                V("earnings_by_daypart", "clusteredColumnChart", 24, 280, 610, 230,
                ...
                V("utilisation_by_zone", "clusteredBarChart", 646, 280, 610, 230,
                  "Utilisation by zone",
                  {"Category": "dim_zone.zone_name", "Y": m("Utilisation %")},
                  {"by": "_Measures.Utilisation %", "dir": "asc"}),
                V("supply_vs_demand", "lineChart", 24, 524, 610, 224,
                  ...
                  {"by": "dim_time_block.hour", "dir": "asc"}),
                V("incentive_cost_by_tier", "clusteredColumnChart", 646, 524, 610, 224,
