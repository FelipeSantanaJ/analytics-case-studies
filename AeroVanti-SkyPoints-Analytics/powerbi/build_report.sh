#!/usr/bin/env bash
# build_report.sh — rebuild the AeroVanti SkyPoints Power BI report end to end.
#
...
if [ "${SKP_ALLOW_DESKTOP:-0}" != "1" ] && command -v tasklist >/dev/null 2>&1; then
  if tasklist 2>/dev/null | grep -Eiq "PBIDesktop\.exe|msmdsrv\.exe"; then
    ...
    echo "       (or SKP_ALLOW_DESKTOP=1 if a different report is open)." >&2
    exit 1
  fi
fi
