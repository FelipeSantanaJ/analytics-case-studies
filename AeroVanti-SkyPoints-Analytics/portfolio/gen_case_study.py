...
Output: portfolio/AeroVanti-SkyPoints-case-study.pdf
...
# ============================================================== CONFIG (per project) ===
PROJECT = "AEROVANTI SKYPOINTS"
AUTHOR = "FELIPE SANTANA · SENIOR DATA & PRODUCT ANALYST"
OUT_NAME = "AeroVanti-SkyPoints-case-study.pdf"

DEEP = (10, 42, 67)          # AeroVanti BLUE_DEEP #0A2A43
ACCENT = (224, 138, 30)      # AeroVanti AMBER #E08A1E

COVER = dict(
    headline=["The test won.", "I still only rolled it out", "to half the program."],
    sub="How I decided which half of the loyalty program to ship — "
        "and which half to hold back.",
    footer="Fictional airline · synthetic data · real methodology",
)

PAGES = [
    dict(section="01 — THE PROBLEM", kind="stat",
         stat="~40%", bold="of issued loyalty miles sit unused.",
         body="Members who don't redeem within 12 months churn at a much higher rate. "
              "AeroVanti needed a way to pull more members into redeeming before that "
              "window closes."),
    dict(section="02 — THE TEST", kind="metrics",
         bold="Lower the redemption threshold. Add more low-cost rewards. "
              "See if redemption goes up.",
         metrics=[
             ("PRIMARY METRIC", "12-week redemption rate — share of members redeeming ≥1×"),
             ("GUARDRAILS", "Revenue / member · point-liability cost / member · lapse rate · "
                             "avg. redemption value"),
         ]),
    dict(section="03 — THE SETUP", kind="cards",
         bold="Why \u201claunch it and see\u201d wasn't good enough here.",
         cards=[("15,000", "ACTIVE MEMBERS"), ("12", "WEEKS"), ("50/50", "RANDOMIZED")],
         body="Member-level randomization — because a before/after comparison can't tell "
              "you whether a lift came from the change, or from the season."),
    dict(section="04 — WHAT HAPPENED", kind="stat_callout",
         stat="+2.7pp", stat_sub="pooled redemption-rate lift · p ≈ 2×10⁻⁶",
         callout="But the effect wasn't the same size in every membership tier — the "
                 "tier interaction was itself statistically significant (p = 0.0007)."),
    dict(section="05 — THE CATCH", kind="bold_callout",
         bold="About half that lift looks like a launch-week novelty bump — "
              "not a durable shift.",
         callout="Average redemption value also dropped. Not a guardrail breach — "
                 "but worth watching after launch."),
]

CALL = dict(
    section="06 — THE CALL",
    headline=["Rolled out to the two tiers", "where the effect was real and durable.",
              "Held off on the rest."],
