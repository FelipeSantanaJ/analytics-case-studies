...
Produces powerbi/LumaBankActivation.SemanticModel/** (TMDL, Parquet import partitions
via a pDataFolder parameter). Measures come from etl/pbi_measures.MEASURE_LIBRARY
(Phase 7 — the _Measures table is empty until then). The .pbip + .Report shell are
created by powerbi/build_report.sh (Phase 8).
"""
...
# committed value — a placeholder, not this dev box's real path (see docs/06 note)
PLACEHOLDER_DATA_FOLDER = f"C:/PATH/TO/REPO/{ROOT.name}/data/curated"

NS = uuid.UUID("6f0a1e42-9c3d-4b77-9a11-2c5d8e7f0b6a")
...
SORT_BY = {
    ...
    "dim_onboarding_step": {"step_name": "step_order"},
}

NO_SUM_COLS = {
    "year", "quarter", "month", "month_key", "month_sort", "iso_week", "day_of_week",
    "date_key", "assign_date_key", "assigned_date_key", "occurred_date_key",
    "opened_date_key", "flagged_date_key", "released_date_key", "signup_date_key",
    "assignment_seq", "experiment_day", "utc_offset_hours", "rollout_pct", "step_order",
}
...
def _load_measure_library():
    try:
        ...
        print(f"  (no measure library yet -- Phase 7: {e})")
        return []
...
def relationships_tmdl(schemas):
    ...
    # date -> dim_date (day-grain facts)
    add("fact_variant_assignment", "assigned_date_key", "dim_date", "date_key")
    add("fact_onboarding_step", "occurred_date_key", "dim_date", "date_key")
    add("fact_activation", "assigned_date_key", "dim_date", "date_key")
    add("fact_transaction_day", "date_key", "dim_date", "date_key")
    add("fact_fraud_event", "flagged_date_key", "dim_date", "date_key")
    add("fact_support_ticket", "opened_date_key", "dim_date", "date_key")
    add("fact_srm_daily", "assign_date_key", "dim_date", "date_key")
    # month-grain fact -> dim_month (dim_date.month_key is not unique, can't sit on "one")
    add("fact_targets_month", "month_key", "dim_month", "month_key")
    # user hub
    for f in ["fact_variant_assignment", "fact_onboarding_step", "fact_kyc_decision",
              "fact_activation", "fact_transaction_day", "fact_fraud_event",
              "fact_support_ticket"]:
        add(f, "user_key", "dim_user", "user_key")
    # other dims
    add("fact_variant_assignment", "app_release_key", "dim_app_release", "app_release_key")
    add("fact_onboarding_step", "step_key", "dim_onboarding_step", "step_key")
    add("fact_kyc_decision", "kyc_outcome_key", "dim_kyc_outcome", "kyc_outcome_key")
    add("fact_activation", "channel_key", "dim_acquisition_channel", "channel_key")
    add("fact_activation", "region_key", "dim_region", "region_key")
    # experiment-arm slicer helper (text key) — related to the two arm-bearing facts
    for f in ["fact_variant_assignment", "fact_activation"]:
...
def expressions_tmdl():
    return "\n".join([
        f'expression pDataFolder = "{PLACEHOLDER_DATA_FOLDER}" meta '
...
def main():
    ...
    # dim_user already carries a real user_key = -1 sentinel row (etl/03_build_curated.py)
    # so orphan KYC/fraud events resolve in the model, not just in this schema sample.

    # dim_month helper — spans only the fact window so MAX(month_key) lands on a
    # month that actually has fact_targets_month rows.
    dd = pd.read_parquet(CURATED / "dim_date.parquet")
    import config as C
    win_lo, win_hi = str(C.WINDOW_START), str(C.WINDOW_END)
    mo = (dd[(dd["date"] >= win_lo) & (dd["date"] <= win_hi)]
