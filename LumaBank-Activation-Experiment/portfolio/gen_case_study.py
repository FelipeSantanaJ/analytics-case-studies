...
Output: portfolio/LumaBank-Activation-case-study.pdf
...
# ============================================================== CONFIG (per project) ===
PROJECT = "LUMABANK ACTIVATION"
AUTHOR = "FELIPE SANTANA · SENIOR DATA & PRODUCT ANALYST"
OUT_NAME = "LumaBank-Activation-case-study.pdf"

DEEP = (10, 51, 58)          # LumaBank PETROL_DEEP #0A333A
ACCENT = (226, 98, 75)       # LumaBank CORAL #E2624B

COVER = dict(
    headline=["The A/B test broke.", "Here's how I salvaged", "the read."],
    sub="A production bug corrupted randomization on day 11 — caught by daily SRM "
        "monitoring, not by luck.",
    footer="Fictional Brazilian neobank · synthetic data · real methodology",
)

PAGES = [
    dict(section="01 — THE PROBLEM", kind="stat",
         stat="Day 11", bold="an unrelated mobile app deploy resets the "
                              "variant-assignment cache.",
         body="Some already-assigned users switch arms mid-journey. New signups fall "
              "into a region-biased fallback. The question stopped being \u201cdid the "
              "redesign work?\u201d and became \u201ccan this experiment still be "
              "trusted?\u201d"),
    dict(section="02 — THE TEST", kind="metrics",
         bold="Onboarding redesign — fewer steps, ID upload deferred — as a 50/50 "
              "user-level A/B test on Day-7 Activation.",
         metrics=[
             ("PRIMARY METRIC", "Day-7 Activation rate — signup to first funded "
                                 "transaction within 7 days"),
             ("GUARDRAILS", "KYC-rejection rate · flagged-fraud rate (30d) — both "
                             "one-sided non-inferiority tests"),
         ]),
    dict(section="03 — THE SETUP", kind="cards",
         bold="Continuous monitoring — not a one-time pre-flight check.",
         cards=[("~135K", "SIGNUPS"), ("6", "WEEK WINDOW"), ("DAILY", "SRM MONITORING")],
         body="Sample-ratio-mismatch, re-derived independently from the raw assignment "
              "log every day. That's what caught the break on day 13."),
    dict(section="04 — WHAT HAPPENED", kind="stat_callout",
         stat="191", stat_sub="contaminated users — plus a fallback skewing treatment "
                               "share 76.5% (Southeast) to 35.3% (North)",
         callout="Truncating or excluding the affected users both left an MDE too wide "
                 "to trust. Restarting clean was the only zero-bias option."),
    dict(section="05 — THE CATCH", kind="bold_callout",
         bold="On the clean re-run, the redesign works — +4.58pp Day-7 Activation "
              "(p ≈ 1×10⁻⁵).",
         callout="But the flagged-fraud guardrail doesn't clear non-inferiority. The "
                 "KYC guardrail does. One clean pass, one not established."),
]

CALL = dict(
    section="06 — THE CALL",
    headline=["A monitored rollout", "with a fraud circuit-breaker —",
              "not an unconditional launch."],
...
def giant_stat(d: ImageDraw.ImageDraw, y: int, stat: str, stat_sub: str | None = None) -> int:
    ...
    if stat_sub:
        ssf = font(SANS, 19)
        for line in wrap(d, stat_sub, ssf, W - 2 * MARGIN):
            d.text((MARGIN, bbox[3] + 14), line, font=ssf, fill=BODY_GRAY)
            bbox = (bbox[0], bbox[1], bbox[2], bbox[3] + 14 + 26)
        return bbox[3] + 10
    return bbox[3] + 30
