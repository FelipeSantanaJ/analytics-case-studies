...
Output:
  docs/VoltEdge-Electronics-Documentation.pdf           (full: 01..09 + walkthrough)
  docs/VoltEdge-Electronics-Documentation-Summary.pdf   (01 context + 05 model + 09 report structure)
"""
...
FULL = ["01_business_context_kpis", "02_data_architecture", "03_data_dictionary",
        "04_etl_pipeline", "05_data_model", "06_dax_measures", "07_report_guide",
        "08_data_quality", "09_report_structure", "walkthrough"]
SUMMARY = ["01_business_context_kpis", "05_data_model", "09_report_structure"]

CSS = """
:root{--navy:#0E2A4E;--blue:#123E6E;--ink:#12212E;--amber:#D99311;--muted:#64707F;
      --line:#DCE4EF;--fill:#EAEFF6;}
...
def render(files, out, title, sub):
    ...
    body = [f'<div class="cover"><div class="k">VOLTEDGE ELECTRONICS</div>'
            f'<h1>{title}</h1><p>{sub}</p>'
            f'<p>Fictional company · synthetic seeded data</p></div>']
    for f in files:
        p = HERE / f"{f}.md"
        if not p.exists():
...
if __name__ == "__main__":
    render(FULL, "VoltEdge-Electronics-Documentation.pdf",
           "Project Documentation", "ETL · data model · DAX · report · analysis walkthrough")
    render(SUMMARY, "VoltEdge-Electronics-Documentation-Summary.pdf",
           "Project Summary", "Business context · data model · report structure")
